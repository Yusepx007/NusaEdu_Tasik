...
export default function LoginPage() {
  ...
  return (
    <div className="page-no-nav fade-in" style={{ display: 'flex', flexDirection: 'column', minHeight: '100dvh', background: 'var(--bg)' }}>
      {/* Header illustration */}
      <div style={{ background: 'linear-gradient(135deg, var(--primary) 0%, #00969870 100%)', padding: '60px 32px 48px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <h1 style={{ color: 'white', fontSize: 24, fontWeight: 800, margin: 0 }}>NusaEdu</h1>
        <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 13, margin: '6px 0 0' }}>Wisata Tasikmalaya Interaktif</p>
      </div>

      {/* Form Card */}
      <div style={{ flex: 1, padding: '32px 24px', display: 'flex', flexDirection: 'column' }}>
        <h2 style={{ fontSize: 22, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 6 }}>Masuk Akun</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: 13, marginBottom: 28 }}>Selamat datang kembali! 👋</p>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          >
            {isLoading ? (
              <><div className="spinner" style={{ width: 20, height: 20, borderWidth: 2 }} /> Masuk...</>
            ) : 'Masuk'}
          </button>
        </form>
        ...
        <div style={{ marginTop: 16, textAlign: 'center' }}>
          <button
            ...
            style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', fontSize: 12, cursor: 'pointer', fontFamily: 'Poppins, sans-serif', textDecoration: 'underline' }}
          >
            Lanjut sebagai tamu
          </button>
        </div>
